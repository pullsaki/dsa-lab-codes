...
void bubblesort(NodeList &list) {
    ...
    for (int i = 0; i < l; i++) { //for every pass, one iteration of this loop is completed
        NodeList::Iterator p = list.begin(); //for every pass, beginning element is stored and moved if it is bigger than adjacent one
        for (int j = 1; j < l - i; j++, ++p) { //for every iteration of this loop, the iterator p should also move right
            NodeList::Iterator s = p; //first equating p to s
            ++s; //then incrementing s... so as to make them adjacent to each other (like j and j + 1 in normal loops)
            if (*p > *s) { //if value at p > value at s then
                Elem tmp = *p; //swap method
                *p = *s;
                *s = tmp;
            }
        }
...
int main() {
    NodeList list;
    for (int i = 0; i < 10; i++) list.insertFront(i + 1);
    NodeList::Iterator p = list.begin();

    for (int i = 0; i < list.size(); i++, ++p) cout << *p << " ";
    cout << endl;
